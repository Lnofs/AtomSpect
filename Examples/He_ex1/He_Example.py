...
from SpectSplit import Zeeman_Main, Normalize, PlotFunction, read_Spectra,  MakeSlider,  plotZfan, Savenpy_Dict, Loadnpy_Dict
import os
import sys
import numpy as np
    ...
    for i, angles in enumerate(polangles):
        ...
        if i == 0:
            taxs = PlotFunction(HePol, HePol_PlotObj, NormalizeSig=True,
                                ...
                                plotlabel=r'$\gamma$' + f'={angles}' + r'$^o$', plotnondip=True)
        else:
            PlotFunction(HePol, HePol_PlotObj,  NormalizeSig=True, axsin=taxs[-1][0], position=[0, i],
                         makefig=False,  plotlabel=r'$\gamma$' + f'={angles}' + r'$^o$', plotnondip=True)
...
if doHeNonPol == True: 
    ...
    for i, Bvals in enumerate(Bmags):
        ...
        colorlist = ['royalblue', 'g', 'r', 'brown', 'm', 'c', 'y', 'black', 'seagreen']
        HeNonPol_PlotVar = [] 
        HeNonPol_PlotVar.append(['royalblue', 2.5, 'solid', 'Exact'])
        HeNonPol_PlotVar.append( [colorlist[1], 1.5, '-.', 'Low'])
        HeNonPol_PlotVar.append( [colorlist[2], 1.5, '--', 'High'])
        ...
        if i == 0:  # In the future, automate this process of the plotting.
            ...
            PlotFunction([HeNonPol['SpecOutLow']], HeNonPol_PlotVar[1],  NormalizeSig=True, axsin=taxs[0][0], position=[0, i],
                         makefig=False,plotwind = InputdeckHeNonPol['plot_window'],  plotnondip=0,plotpol=0)
            PlotFunction([HeNonPol['SpecOutHigh']], HeNonPol_PlotVar[2],  NormalizeSig=True, axsin=taxs[0][0], position=[0, i],
                         makefig=False,plotwind = InputdeckHeNonPol['plot_window'],  plotnondip=0,plotpol=0)
     
        else:
            PlotFunction(HeNonPol, HeNonPol_PlotVar[0],  NormalizeSig=True, axsin=taxs[i][0], position=[0, i],
                            makefig=False,  plotlabel=f'B={Bvals}T', plotnondip=0, plotpol=0)         
            PlotFunction([HeNonPol['SpecOutHigh']], HeNonPol_PlotVar[2],  NormalizeSig=True, axsin=taxs[i][0], position=[0, i],
                         makefig=False,plotwind = InputdeckHeNonPol['plot_window'],  plotnondip=0,plotpol=0)
            
            PlotFunction([HeNonPol['SpecOutLow']], HeNonPol_PlotVar[1],  NormalizeSig=True, axsin=taxs[i][0], position=[0, i],
                         makefig=False,plotwind = InputdeckHeNonPol['plot_window'],  plotnondip=0,plotpol=0)
    ...
    if savepdf:
        plt.savefig('He_NonPolRegime.pdf', dpi=savedpi)
    if savepng:      
        plt.tight_layout(rect=[0, 0, 1, .9])
...
if doHeNonPol2 == True: 
    ...
    if savepdf:
        plt.savefig('He_NonPol2.pdf', dpi=savedpi)
    if savepng:      
        plt.tight_layout(rect=[0, 0, 1, .9])
        plt.rcParams['axes.titlepad'] = 405
...
if doHeNonPol3 == True:
    ...
    if savepdf:
        plt.savefig('He_NonPol3.pdf', dpi=savedpi)
    if savepng:      
...
if fanplot == True:
    ...
    InputdeckHeNonPolFan['Bmag'] = np.linspace(0, 2, 100)
    Brange = np.linspace(0, 2, 100)
    flags = 'HGL'
    Zfantest = plotZfan(InputdeckHeNonPolFan, flags=flags)
    # plt.ylim(169083.4, 169090)

    # plt.tight_layout()
    if savepdf:
        plt.savefig(f'He_Fan.pdf', dpi=savedpi)
    if savepng:
...
if fanplot2 == True:
    ...
    InputdeckHeNonPolFan['Bmag'] = np.linspace(0, .025, 1000)
    # Brange = np.linspace(0, .25, 100)
    flags = 'HEL'
    Zfantest = plotZfan(InputdeckHeNonPolFan, flags=flags)
    # plt.ylim(169083.4, 169090)
    if savepdf:
        plt.savefig(f'He_FanE.pdf', dpi=savedpi)
    if savepng:
