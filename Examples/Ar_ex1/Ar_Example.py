...
from SpectSplit import Zeeman_Main, Normalize_Scale,Normalize,PlotFunction,read_Spectra
...
for i,Bvals in enumerate(Ar_B):
    ...
    axs.plot(Ar2['SpecOut'][0],Normalize_Scale(Ar2['SpecOut'][1] , scaling = np.max(Ar_Spec['signals'][i][690:])) ,  label=InputdeckAr2['plottitle'])# ,color =colorlist[i])
