...
def PlotFunction_old(Plotobject,plot_vars,plottitle = '', plotwind = [],SpectrumPlot = [], Nrows = 1, NormalizeSig = False, NormalizeScale = 1, makefig =True):
        ...
        for i,(tplotobj,plotvars) in enumerate(zip(tPlotObj,plot_vars)):
            if NormalizeSig == True:
                if len(np.shape([NormalizeScale])) == 1: #Condition checks if the user input a single value for the NormalizeScale or a list. If it's a list, then iterate over the entries to scale as requested.
                    plt.plot(tplotobj[0],NormalizeScale(tplotobj[1],scaling=NormalizeScale),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
                else:
                    plt.plot(tplotobj[0],NormalizeScale(tplotobj[1],scaling=NormalizeScale[i]),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
