...
def plotarray(shape, figuresize=(10, 16)):
...
def PlotFunction(Plotobject, plot_vars,  plottitle='', plotwind=None, SpectrumPlot=[], Shape=[1, 1], position=[0, 0],
                 axsin=[], NormalizeSig=False, NormalizeScale=1, makefig=True, figuresize=(10, 16), plotpol=True,
                 plotlabel='', plotnondip=False, dotwin=False, marker = None):
    '''
    ...
    try:
        if 'plot_window' in Plotobject['Input']:
    ...
    try:
        ...
        if makefig == True:
            ...
            try:
                taxs[position[0], position[1]].plot(SpectrumPlot[0], Normalize(SpectrumPlot[1]), color='black',linestyle = ':', linewidth=3.5, label=f'{SpectrumPlot[2]}')
            except:
                taxs[position[0], position[1]].plot(SpectrumPlot[0], Normalize(SpectrumPlot[1]), color='black',linestyle = ':', linewidth=3.5, label='Spectrometer')
    ...
    if SpectrumPlot != []:
        ...
        try:
            taxs[position[0], position[1]].plot(SpectrumPlot[0], Normalize(SpectrumPlot[1]), color='black',linestyle = ':', linewidth=3.5, label='Spectrometer')
            taxs[position[0], position[1]].text(0.05, 0.90, f'{plotlabel}', verticalalignment='top',
                                                horizontalalignment='left', transform=taxs[position[0], position[1]].transAxes)
        except:
            taxs.plot(SpectrumPlot[0], Normalize(SpectrumPlot[1]), color='black',linestyle = ':', linewidth=3.5, label='Spectrometer')
            taxs.text(0.05, 0.90, f'{plotlabel}', verticalalignment='top',
    ...
    try:
        ...
        if plotpol:
            ...
            if plotnondip:
                twinaxis.stem(Plotobject['other_sticks'][0], Plotobject['other_sticks'][1], linefmt='C4', markerfmt='*', basefmt=" ",  label=r'$\Delta m >1$')
...
def MakeSlider(Inputdict, Spectra, plotwind=None, do_sticks=True, do_Polplot=True, banglevary=False, polvary=False, do_other = False):
    ...
    line, = axs.plot(Mycalc0['SpecOut'][0],
                     ...
                     lw=2.5, label="Exact", linestyle=':', alpha=0.9)
    ...
    if 'DoLowSig' in Inputdict:
        ...
        if Inputdict['DoLowSig'] in possibleanswers:
            ...
            line2, = axs.plot(Mycalc0['SpecOutLow'][0],
                              ...
                              lw=1, alpha=.9, label="Lowfield", linestyle=(5, (5, 2, 1, 2)), marker='D', markevery=50, markersize=5)
            numcols = 4
    else:
        line2 = None
    ...
    if 'DoHighSig' in Inputdict:
        ...
        if Inputdict['DoHighSig'] in possibleanswers:
            ...
            line3, = axs.plot(Mycalc0['SpecOutHigh'][0],
                              ...
                              lw=1, alpha=.9, label="Highfield", linestyle=(5, (5, 2, 1, 2)), marker='x', markevery=50, markersize=5)
            numcols = 4
    ...
    Res_Slider = Slider(axampR, r"$W_i$ (nm)", 0.005, 0.1, valstep=0.001, valinit=init_Res,
                        orientation="vertical")
...
def Zeeman_func(Level_in, Binput, Lowfield=False, Highfield=False):
    '''
...
def FastZfan(States_in, Bfield, g_I=0, A_HFS=0):
    #
    ...
    for i, states in enumerate(States_in[-1]):
        if g_I == 0:
            ...
            thisZeemanEnergyH = ZeemanHigh(states[5], states[3], Bfield) + States_in[1][i][i]
    ...
    if 'I_spin' in Inputdeck:
        ...
        if 'G' in flags:
            ...
            if 'H' in flags:
                HighG = []
                for Bvals in Bfield_ext:
                    HighG.append(FastZfan(Ground_Level, Bvals)[1])
        ...
        if 'E' in flags:
            ...
            if 'L' in flags:
                LowE = []
                for Bvals in Bfield_ext:
                    LowE.append(FastZfan(Excited_Level, Bvals)[1])

                output['LowE'] = np.array(LowE).T
            if 'H' in flags:
                HighE = []
                for Bvals in Bfield_ext:
                    HighE.append(FastZfan(Excited_Level, Bvals)[1])
...
def plotZfan(Inputdeck, flags='', savefig=False, figname='placeholder'):
    ...
    if 'G' in flags:
        ...
        figs, axs = plt.subplots(figsize=(10, 16))
        plt.rcParams.update({'font.size': 18})

        axs.plot(Brange[0], Zfantest['G'][0][0], color='blue',  linewidth=1.25, label="Exact", alpha=0.5)  # This makes a single point for the purpose of the legend.
        for things in (Zfantest['G']):
            axs.plot(Brange, things, color='blue', linewidth=1.25, alpha=0.5)  # , label = "Ground")

        if 'L' in flags:
            for thingsL in (Zfantest['LowG']):
                plt.plot(Brange, thingsL, color='maroon', linestyle=':', alpha=0.5)  # , label = "Ground_Low")
            axs.plot(Brange[0], Zfantest['LowG'][0][0], color='maroon', linestyle=':', label="Ground Low")  # This makes a single point for the purpose of the legend.
        if 'H' in flags:
            for thingsH in (Zfantest['HighG']):
                plt.plot(Brange, thingsH, color='green', linestyle='-.', alpha=0.5)  # , label = "Ground_Low")
            axs.plot(Brange[0], Zfantest['HighG'][0][0], color='green', linestyle='-.', label="Ground High")  # This makes a single point for the purpose of the legend.
    ...
    if 'E' in flags:
        ...
        axs.plot(Brange[0], Zfantest['E'][0][0], color='blue',  linewidth=1.25, label="Exact", alpha=0.5)  # This makes a single point for the purpose of the legend.
        for things2 in Zfantest['E']:
            axs.plot(Brange, things2, color='blue', linewidth=1.25, alpha=0.5)  # , label = "Ground")
        if 'L' in flags:
            for thingsL in (Zfantest['LowE']):
                axs.plot(Brange, thingsL, color='maroon', linestyle=':', alpha=0.5)  # , label = "Ground_Low")
            axs.plot(Brange[0], Zfantest['LowE'][0][0], color='maroon', linestyle=':', label="Excited Low")  # This makes a single point for the purpose of the legend.
        if 'H' in flags:
            for thingsH in (Zfantest['HighE']):
                axs.plot(Brange, thingsH, color='green', linestyle='-.', alpha=0.5)  # , label = "Ground_Low")
            axs.plot(Brange[0], Zfantest['HighE'][0][0], color='green', linestyle='-.', label="Excited High")  # This makes a single point for the purpose of the legend.
...
def Zeeman_Main(Inputdeck, **kwargs):
    ...
    # If there is no nuclear spin in the input deck, do a similar calculation but no hpyerfine splitting ins included nor nuclear zeeman splitting.
        ...
        if 'DoHighSig' in Inputdeck:
            if Inputdeck['DoHighSig'] == 'N' or Inputdeck['DoHighSig'] == 'No':
                ...
                Ground_Level = stateprocess(L_ground, S_ground, E_ground, Bfield_ext, sortE=sortE)
                Excited_Level = stateprocess(L_excited, S_excited, E_excited, Bfield_ext, sortE=sortE)
                Ground_Zeeman_High = Zeeman_func(Ground_Level, Bfield_ext, Highfield=True)
                Excited_Zeeman_High = Zeeman_func(Excited_Level, Bfield_ext, Highfield=True)
    ...
    if 'DoHighSig' in Inputdeck:
        ConvolvedSpectLow, redsticksLow, binwindsLow = Convol_Spect(data['wave_air_high'], data['signal_high'],
                                                                    Inputdeck['spec_window'], Inputdeck['amu'], stepsize,
                                                                    Temperature_in=convtemp, functiontype=Convtype,
                                                                    wind_size=fxnwindow, Skewness_consts=SkewedConstants, ionvel=ion_vel, specres=specres, bidir=bidirect)

        data['SpecOutHigh'] = ConvolvedSpectLow
        data['reduced_sticksHigh'] = redsticksLow
        # data['bin_windows'] = binwindsLow
