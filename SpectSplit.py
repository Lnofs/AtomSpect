...
def air_to_vac(wavein):
    ...
    return wavelengths 
#%%Plot
def plotarray(shape,figuresize= (14,8)):
    fig = plt.figure(figsize=figuresize)#,layout="constrained")
    ...
    fig.supylabel('Nomralized Intensity (arb)')
    return fig,plotmat




def PlotFunction(Plotobject,plot_vars,plottitle = '', plotwind = [],SpectrumPlot = [], Shape =[1,1],position=[0,0],
                  axsin=[], NormalizeSig = False, NormalizeScale = 1, makefig =True,figuresize=(14,8)):
    '''
    This plot function is robust and can handle any combination of: Zeeman_Main (dictionary) output object, 
    ...
    #plot_vars are [color, linewidth, linestyle, label,polkwargs]
    ...
    try:
        ...
        if makefig ==True:
            tfig,taxs = plotarray(Shape,figuresize = figuresize)
        else:
            taxs = axsin
        ...
        if SpectrumPlot!=[]:
            ...
            taxs[position[0],position[1]].plot(SpectrumPlot[0], SpectrumPlot[1],color='black',linewidth = 1.5, label='Spectrometer Data' )
        ...
        for i,(tplotobj,plotvars) in enumerate(zip(tPlotObj,plot_vars)):
            if NormalizeSig == True:
                if len(np.shape([NormalizeScale])) == 1: #Condition checks if the user input a single value for the NormalizeScale or a list. If it's a list, then iterate over the entries to scale as requested.
                    taxs[position[0],position[1]].plot(tplotobj[0],NormalizeScale(tplotobj[1],scaling=NormalizeScale),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
                else:
                    taxs[position[0],position[1]].plot(tplotobj[0],NormalizeScale(tplotobj[1],scaling=NormalizeScale[i]),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
                  
            else:
                taxs[position[0],position[1]].plot(tplotobj[0],tplotobj[1],color=plotvars[0],linestyle=plotvars[2],linewidth = float(plotvars[1]), label=plotvars[3], alpha = 0.95)
    ...
    if SpectrumPlot!=[]:
        ...
        try:
            ...
            taxs.plot(SpectrumPlot[0], SpectrumPlot[1],color='black',linewidth = 1.5, label='Spectrometer Data' )
    ...
    if len(np.shape(tPlotObj)) == 2: #The first case is for a single spectra object or dictionary. A dictionary returns np.shape(dict) = (), which has a length of 0.
        if NormalizeSig == True: #Check if Normalize Sig flag is True, if so then use the Normalize function to scale the results.
            taxs[position[0],position[1]].plot(tPlotObj[0],Normalize(tPlotObj[1],scaling=NormalizeScale),color=plot_vars[0],linestyle=plot_vars[2],linewidth = plot_vars[1], label=plot_vars[3], alpha = 0.95)
        else:
            taxs[position[0],position[1]].plot(tPlotObj[0],tPlotObj[1],color=plot_vars[0],linestyle=plot_vars[2],linewidth = plot_vars[1], label=plot_vars[3], alpha = 0.95)
        ...
        for i,(plotobj,plotvars) in enumerate(zip(tPlotObj,plot_vars)):
            ...
            if NormalizeSig == True:
                try:
                    try:
                        taxs[position[0],position[1]].plot(t_plotobj[0],Normalize(t_plotobj[1],scaling=NormalizeScale[i]),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
                    except:
                        taxs.plot(t_plotobj[0],Normalize(t_plotobj[1],scaling=NormalizeScale[i]),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)

                except:
                    try:
                        taxs[position[0],position[1]].plot(t_plotobj[0],Normalize(t_plotobj[1],scaling=NormalizeScale),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
                    except:
                        taxs.plot(t_plotobj[0],Normalize(t_plotobj[1],scaling=NormalizeScale),color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)

            else:
                try:                    
                    taxs[position[0],position[1]].plot(t_plotobj[0],t_plotobj[1],color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
                except:
                    taxs.plot(t_plotobj[0],t_plotobj[1],color=plotvars[0],linestyle=plotvars[2],linewidth = plotvars[1], label=plotvars[3], alpha = 0.95)
    ...
    try:
        ...
        # taxs[0][0].legend([],bbox_to_anchor=(1.04, 0.5), loc="center left")#, ncols=2,borderaxespad=0)
    
        # taxs[position[0],position[1]].legend(loc='best')
        # plt.tight_layout()
        # plt.show()
        ...
        if makefig==True:
            ...
            # plt.tight_layout()
            tfig.legend(*zip(*unique),bbox_to_anchor=(.95,.5), loc="outside center right")
            # plt.tight_layout()
...
def PlotPolarization(Plotobject,plot_vars,plottitle = '', plotwind = [],SpectrumPlot = [], Shape =(1,1),position=[0,0],
                  axsin=[], NormalizeSig = False, NormalizeScale = 1, makefig =True,figuresize=(14,8)):
    #Plotobjects can [[[waves],[signals]] or a list of multiple plot objects. [[[wave1],[sig1]] ,[[wave2],[sig2]] ] etc
    #plot_vars are [color, linewidth, linestyle, label,polkwargs]
    #verymuch want to add the ability to make multiple plots in the same window using this function and indexing the shape of the multiplot.     
    ...
    #First check if the plotobject is inhomogogenous, which is to be the case should the user mix LZeeman output objects and other spectra.
    #This function only serves to plot based on polarization status, so don't need to filter out arrays.
    ...
    try:
        ...
        #If the np.shape fails a second time, then it means the spectra are just of different lengths. This is fine, so we just iterate through them and plot
        if makefig ==True:
            fig,taxs = plotarray(Shape,figuresize = figuresize)
        else:
            taxs = axsin
...
def GaussianInstrum(x, x0, Temp_in, amu,pixel_size = 0,v_ion = 0,bidir=False):
    ...
    W = np.sqrt((pixel_size*1e-9)**2 + 8*np.log(2)*tdop)
    if bidir==True:
...
def Voigt(x,x0,Temp_in,amu,pixel_size=1,v_ion=0,bidir = False):
    ...
    if bidir==True:        
        try:#First try having v_ion be an array, so that the counter-streaming velocities can be different.
            ...
            mean1 = x0*(1 +v_ion/c_light)
            mean2 = x0*(1 -v_ion/c_light)
        V1 = scp.special.voigt_profile(x-mean1,pixel_size,tdop)
        V2 = scp.special.voigt_profile(x-mean2,pixel_size,tdop)
        V = (V1 + V2)/2
    else: #If bidir is False, do normal doppler shifted.
        mean = x0*(1 +v_ion/c_light)    
...
# def Normalize_Scale(list_in ,scaling=1.0):
#     '''
...
def polarization(mupper, mlower, Btheta,gamma=0,Pol_Filter=False,T_Alpha = 1, T_Beta=0):
    ...
    # eps11 = np.sqrt(0.5*(np.cos(theta_rad)**2) * (ealph**2) + (ebeta**2 ))
...
def Zeemansig_Low(Sg, Eg, Se, Ee, Bangle, gamma, Filter = False):
    #Need to incorporate this to not require the pre-processing, but it's a good start
    #Want stateprocess included in here so it's a standalone thing given then NIST inputs.
    bjlist = [[x,y] for y in Se for x in Sg] #This list will be [[bigstatesG], [bigstatesE]]
    
    
    #need to define the polarization effect from the filter.
    rad = np.zeros((len(bjlist)))
    for i in range(len(bjlist)):
        # delta_am = np.abs(m_upper - m_lower)
    ...
    # afcf = np.kron(np.array(LZE[1])[:,::-1],np.array(LZG[1]))
    afcf = np.eye(len(Eg)*len(Ee))
    # afcf = np.kron(np.array(LZE[1]),np.array(LZG[1]))
    ...
    wave_vac = np.array( [1e7/(y-x) for y in Ee for x in Eg])
    ...
    #Eventually would like a way to convert this data to a more human readable form 
        ...
        for i,states1 in enumerate(sortedstates):
            ...
            for j,states2 in enumerate(sortedstates):
                ...
                w3mat[i,j] = CG1
        return [w3mat,E_SO_cm_Mat,E_cm_flat,sortedstates]
        ...
        for i,states1 in enumerate(bigstatelist):
            ...
            for j,states2 in enumerate(bigstatelist):
                ...
                if states1[2]!= states2[2]:
                    ...
                    CG1 = CGw3(states2[2],states2[3],states2[4],states2[5], states1[0],states1[1])
                w3mat[i,j] = CG1
        return [w3mat,E_cm0_Mat,E_cm0_flat,bigstatelist]
...
def Zeeman_signal(LevelG,LZG,LevelE,LZE,Bangle=90,gamma = 0,Filter=False):
    ...
    Signalcoeff = []
    for i in range(len(bjlist)):
        Signalcoeff.append(rad[i][0]*dipolestr(bjlist[i][0],bjlist[i][1]))
    Signalcoeffa = np.array(Signalcoeff)
...
def FastZfan(States_in,Bfield, g_I=0):
    #
    ...
    for i,states in enumerate(States_in[-1]):
        if g_I ==0:
            ...
            thisZeemanEnergy = Zeemanlow(thislandegF,states[1],Bfield) + States_in[1][i][i]  

        else:
            thislandegF = lande_gf(states[0],states[4], states[2], states[6],states[8],g_I) #Calculate lande gf factor for the given transition
            thisZeemanEnergy = Zeemanlow(thislandegF,states[1],Bfield) +  States_in[2][i] + States_in[1][i][i]  
        Zfans.append(thisZeemanEnergy)
...
def ZeemanFan(Inputdeck,flags=''):
    ...
    if 'I_spin' in Inputdeck:
        ...
        output['gi']= thisg_I
        if 'G' in flags: 
            Ground_Level_HFS = stateprocess_HFS(L_ground, S_ground, I_spin, HFSConstsG, thisg_I, E_ground, 1)
            Ground_Zeeman_HFS= HZeeman_HFS(Ground_Level_HFS, thisg_I)
            # output['Glevel'] = Ground_Level_HFS
            # output['GHZeem'] = Ground_Zeeman_HFS
            ...
            for Bvals in Bfield_ext:
                ...
                if 'PsiHFS' in Inputdeck: 
                    ...
                    tmat = np.asmatrix(Ground_Zeeman_HFS['E0'] +  Ground_Zeeman_HFS['EHF0'] +  Bvals*Ground_Zeeman_HFS['H_Z']*muBcm)
                #Fan works with diag HFS0, but other spectra matches with PsiHFS?
            ...
            if 'L' in flags:
                LowG = []
                for Bvals in Bfield_ext:
                    LowG.append(FastZfan(Ground_Level_HFS,Bvals,g_I=thisg_I))
            
                output['LowG'] = np.array(LowG).T

        if 'E' in flags: 
            Excited_Level_HFS = stateprocess_HFS(L_excited, S_excited, I_spin, HFSConstsE, thisg_I, E_excited, 1)
            Excited_Zeeman_HFS= HZeeman_HFS(Excited_Level_HFS, thisg_I)

            EigE = []
            for Bvals in Bfield_ext:
                # Excited_Level_HFS = stateprocess_HFS(L_excited, S_excited, I_spin, HFSConstsE, thisg_I, E_excited, Bvals,sortE=True)
                # Excited_Zeeman_HFS= HZeeman_HFS(Excited_Level_HFS, thisg_I)
                if 'PsiHFS' in Inputdeck: 

                    tmat = np.asmatrix(Excited_Zeeman_HFS['E0'] +  Excited_Zeeman_HFS['PsiHFS'] +  Bvals*Excited_Zeeman_HFS['H_Z']*muBcm)
                else:
                    tmat = np.asmatrix(Excited_Zeeman_HFS['E0'] +  Excited_Zeeman_HFS['EHF0'] +  Bvals*Excited_Zeeman_HFS['H_Z']*muBcm)
        
                EigE.append(np.linalg.eigh(tmat)[0])
            output['E'] =np.array(EigE).T
            output['Elevel'] = Excited_Level_HFS
            output['EHZeem'] = Excited_Zeeman_HFS
            if 'L' in flags:
                LowE = []
                for Bvals in Bfield_ext:
                    LowE.append(FastZfan(Excited_Level_HFS,Bvals,g_I=thisg_I))
        ...
        if 'G' in flags: 
            ...
            if 'L' in flags:
                LowG = []
                for Bvals in Bfield_ext:
                    LowG.append(FastZfan(Ground_Level,Bvals))
        ...
        if 'E' in flags: 
            ...
            if 'L' in flags:
                LowE = []
                for Bvals in Bfield_ext:
                    LowE.append(FastZfan(Excited_Level,Bvals))
...
def plotZfan(Inputdeck,flags='',savefig = False, figname = 'placeholder'):
    '''
    ...
    if 'G' in flags:
        ...
        plt.plot(Brange[0],Zfantest['G'][0][0], color ='blue',  linewidth = 1.25, label = "Ground", alpha=0.5) #This makes a single point for the purpose of the legend.
        for things in (Zfantest['G']):
            plt.plot(Brange,things, color ='blue' , linewidth = 1.25, alpha=0.5)#, label = "Ground")
            
        if 'L' in flags:
            for things in (Zfantest['LowG']):
                plt.plot(Brange,things, color ='maroon' , linestyle=':', alpha=0.5)#, label = "Ground_Low")
            plt.plot(Brange[0],Zfantest['LowG'][0][0], color ='maroon', linestyle=':', label = "Ground_Low") #This makes a single point for the purpose of the legend.
             
    if 'E' in flags:
        plt.figure()
        
        plt.plot(Brange[0],Zfantest['E'][0][0], color ='blue',  linewidth = 1.25, label = "Excited", alpha=0.5) #This makes a single point for the purpose of the legend.
        for things2 in Zfantest['E']:
            plt.plot(Brange,things2, color ='blue' , linewidth = 1.25, alpha=0.5)#, label = "Ground")
        if 'L' in flags:
            for things in (Zfantest['LowE']):
                plt.plot(Brange,things, color ='maroon' , linestyle=':', alpha=0.5)#, label = "Ground_Low")
            plt.plot(Brange[0],Zfantest['LowE'][0][0], color ='maroon', linestyle=':', label = "Excited_Low") #This makes a single point for the purpose of the legend.
...
def Zeeman_Main(Inputdeck,**kwargs):
    ...
    Inputdeck :     InputdeckExample = {
                     ...
                     'Calc_HFS_Vals': [False,False], #Optional. Passed regarding wether HFS constants A and B should be approximated for values of 0 above.
                     'ion_vel'
                     'PsiHFS'
                     }
    ...
    ConvolvedSpect, redsticks, binwinds = Convol_Spect(data['wave_air'], data['signal'], 
                                                       ...
                                                       wind_size=fxnwindow, Skewness_consts=SkewedConstants,ionvel=ion_vel,specres = specres)    
...
def Convol_Sticks(x0,xwind,Temp_in, amu_in, stepsize_nm, function = "", Skew_consts=[1,1],vel_ion=0,specres = 10):
    '''
    ...
    match function:
        case 'Skewed':            
            ...
            tlorry = Lorrentzian(waveout,x0,pixel_size =stepsize_nm,Skewness=Skew_consts)
        ...
        case 'Lorrentzian':
            ...
            tlorry = Lorrentzian(waveout,x0,pixel_size =stepsize_nm,Skewness=[1,1])
            tlorry[tlorry>1] =1 #Sometimes an odd range for the lorrentzian causes the central peak to be twice as high for a single point, this should correct for that.
            specout = tlorry
        case 'Gaussian':
            tlorry = Gaussian(1e-9*waveout,1e-9*x0,Temp_in,amu_in,v_ion=vel_ion)
            # tlorry[tlorry>1] =1 #Sometimes an odd range for the lorrentzian causes the central peak to be twice as high for a single point, this should correct for that.
            specout = tlorry
        case 'GaussianInstrum':
            tlorry = GaussianInstrum(1e-9*waveout,1e-9*x0,Temp_in,amu_in,pixel_size =stepsize_nm,v_ion=vel_ion)
            # tlorry[tlorry>1] =1 #Sometimes an odd range for the lorrentzian causes the central peak to be twice as high for a single point, this should correct for that.
            specout = tlorry 
        case 'Voigt':
            
            tlorry = Voigt(1e-9*waveout,1e-9*x0,Temp_in,amu_in,pixel_size =1e-9*stepsize_nm,v_ion=vel_ion)
            # tlorry[tlorry>1] =1 #Sometimes an odd range for the lorrentzian causes the central peak to be twice as high for a single point, this should correct for that.
            specout = tlorry 
...
def Convol_Spect(waves_in, signal_in, wave_wind, atomic_weight_amu, resolution_nm,Temperature_in=300, functiontype = "", wind_size =.1, Skewness_consts = [1,1],ionvel = 0,specres=10):
    ...
    for sticks, sigs, winds in zip(tbins['binned_waves'], tbins['binned_signals'], tbins['bin_windows']):
        ...
        for wavs, sigvals in zip(sticks,sigs):
            tspec = Convol_Sticks(wavs, [winds[0],winds[1]], Temperature_in, atomic_weight_amu,resolution_nm,function=functiontype, Skew_consts=Skewness_consts,vel_ion=ionvel,specres=specres)
